...
DWORD WINAPI MultiplyBlock(LPVOID lpParam) {
    ...
    int startRow = data->rowBlock * data->blockSize;
    int startCol = data->colBlock * data->blockSize;
    
    int endRow = std::min(startRow + data->blockSize, data->A->getRows());
    int endCol = std::min(startCol + data->blockSize, data->C->getCols());
...
public:
    ...
    void multiplyMatricesParallel() {
        ...
        threadHandles.reserve(numRowBlocks * numColBlocks);
        
        for (int i = 0; i < numRowBlocks; ++i) {
            for (int j = 0; j < numColBlocks; ++j) {
                ThreadData* data = new ThreadData{&A, &B, &C, blockSize, i, j, &cs};
